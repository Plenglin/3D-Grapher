...
def generate3dPoints(func, xMin, xMax, yMin, yMax, xStep, yStep):
    ...
    while x <= xMax:
        ...
        while y <= yMax:
            try:
                a.append(matrix.columnvec(x, func(x, y), y, 1))
            except ZeroDivisionError:
                a.append(matrix.columnvec(x, 0, y, 1))
            y += yStep
        values.append(a)
        x += xStep
    return values
...
def render(display, points):
    ...
    z = transformation * matrix.columnvec(0, 20, 0, 1)
    y = transformation * matrix.columnvec(0, 0, 20, 1)
